...
if(isset($submit)){
	...
	$date = date('Y-m-d');
	$time = date('h:i sa');
	$query = mysqli_query($conn,"INSERT INTO  product_purchase (pur_id, pur_id1, product, quantity, price, paid_status, dateofpurchase, sup_id, comments, date) VALUES('$pur_id','$pur_id1','$product','$quantity','$price','$paid_status','$dateofpurchase','$sup_id','$comments','$date')");
	
	echo'<script>alert("Successfully Added..!");window.location.href="add_product_purchase.php";</script>';
}
?>
<!doctype html>
...
<?php headertemplate(); ?>
  ...
  <body class="">
                  ...
                  <div class="card-body">
                    <div class="row">
                     <div class="col-xl-4">
                      <div class="row">	
						<?php
					$query1= mysqli_query($conn,"SELECT * FROM product_purchase ORDER BY `id` DESC");
					$rows = mysqli_fetch_assoc($query1);
					$pur_id1= $rows['id'];
					$pur_id2=$pur_id1+1;
				  ?>
                            <input type="hidden" name="pur_id" class="form-control" Value="<?php  printf("PUR%06d", $pur_id2);?>" autocomplete="off"/ readonly>
                            <input type="hidden" name="pur_id1" class="form-control" value="<?php echo $pur_id2;?>" autocomplete="off"/>
					  ...
					  <div class="col-md-6 col-xl-12">
						...
						<div class="mb-3">
                          <div class="mb-3">
                            ...
                            <input type="text" name="quantity" class="form-control" autocomplete="off" required/>
                          </div>
                        </div>
                      </div>
	...
	$time = date('h:i sa');
	?>
                              <input id="calendar-simple" name="dateofpurchase" type="date" min="<?php echo $date; ?>" value="<?php echo $date; ?>" class="form-control mb-2" placeholder="Select a date" required />
